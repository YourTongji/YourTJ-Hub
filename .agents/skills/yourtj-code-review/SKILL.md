...
- 测试要覆盖**代码实际存在的分支**：条件/错误返回/回退路径都要有断言，参考
  `app/migration/migration_course_review_upgrade_test.go`（`TestCourseReviewUpgradeCrashWindowRollback`、
  `TestCourseReviewUpgradeHalfMigratedRestore`）与 `app/service/contentdeleteservice/reward_consistency_test.go`
  （`TestDeletePostByUserRollsBackRewardWhenContentDeleteFails` 等双向往滚断言）。
- 边界与量级敏感点必须显式测，参考 `app/migration/course_aggregation_upgrade_test.go`
  （`TestCourseAggregationUpgradeLargeBatch`：跨批边界 0/499/500/777/1099 断言）与
  `app/models/forum/course/terms_pg_test.go`（`TestListDistinctTermsOnPostgreSQL`：仅 PostgreSQL 暴露的方言分支）。
- 失败路径可观测：错误有断言（不只看日志），迁移/批量任务失败后的重试与自愈有测试。参考
  `app/migration/errors_test.go`（`TestDeferredClassifiesMigrationOutcomes`：hard/deferred 分类）、
  `app/migration/version_lock_test.go`（`TestSQLiteMigrationLockTakesOverExpiredLease`）与
  `app/service/wikiservice/sync_test.go`（`TestUnshallowMarkerSurvivesProjectionFailure`、
  `TestApplyRepoToDBInvalidNestedPathFailsFast`）。
- 数据量要贴近真实，例如 `app/migration/course_aggregation_upgrade_test.go` 的
  `TestCourseAggregationUpgradeLargeBatch`（1100 行 > 批大小 500，强制 keyset 分页分支，
  `sqlite :memory:` 即可复现）。
