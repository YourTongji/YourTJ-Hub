...
sync_one() {
  ...
  if [ "$label" = "sqlite" ] && [ "$(sqlite3 "$TMP" "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='campus_identity_bindings';")" = "1" ]; then
    sqlite3 "$TMP" "DELETE FROM campus_identity_bindings; VACUUM;"
  fi
  mkdir -p "$(dirname "$dst")"
...
SQL
  ...
  docker exec -e MAIN_PG="$MAIN_PG" -e DEV_PG="$DEV_PG" yourtj-postgres sh -c \
    'pg_dump --exclude-table-data=public.campus_identity_bindings -U yourtj -d "$MAIN_PG" | psql -U yourtj -d "$DEV_PG"' >/dev/null
