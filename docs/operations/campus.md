...
## Private timetable map and classroom schedule source

`Current` privacy boundary: `/map?mine=1` is an intent-only server-rendered page
request. It does not put a course ID, course name, campus, or room in the URL. The
map response is `private, no-store`, its server-rendered props are empty, and its
canonical URL is `/map`.

`Partial` user flow: the `/campus` Today section header has one generic map link
beside the week-view link. It opens the original `/map` with its timetable menu
tab selected; the map canvas, campus selector, and map controls remain available.
The browser calls the existing authenticated campus
status, today, calendar, and timetable APIs. The map checks binding state before
showing its in-memory list, rechecks status when the page regains focus or
visibility, and clears stale data on unmount or binding-version change. It does not
persist personal timetable data. A separate course-search mode in the same map menu
uses the existing PK scheduler `courses-by-time` and batched `course-details`
interfaces to display synchronized teaching arrangements by term, weekday, period
group and teaching week. A response's
`updatedAt` is generated by YourTJ when it normalizes that response. It is a local
query/projection timestamp, not an upstream record `updateTime` or evidence of
source freshness; the map does not display it as a freshness claim. Room strings are
matched against uniquely named map features and remain unpinned when ambiguous.

`Partial`: the map reuses the existing PK scheduler read interfaces and their
course-module data. It does not add a new teaching-resource client, credential, or
upstream endpoint. This data contains teaching arrangements with free-text room
locations; it does not provide authoritative building/classroom IDs or guarantee
comprehensive schedules. Never infer room occupancy, availability, opening, or
reservation eligibility from these arrangements or from a missing record.

`Decision needed`: broader source coverage, school-verified building identifiers,
and source freshness/provenance semantics remain outside the current feature. Any
future authoritative room-schedule source must have explicit platform authorization
and sample verification for building/classroom coverage, pagination, date/term
agreement, reschedules, deletion markers, and duplicate handling.
