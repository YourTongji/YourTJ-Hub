...
persist personal timetable data or read the PK scheduler/local plans. A response's
...
source freshness; the map does not display it as a freshness claim.

`Decision needed`: full teaching-building/classroom schedules are not enabled. The
current `[campus]` client configuration supports the OneTJ identity flow; it does
not establish application authorization for teaching-resource APIs using client
credentials. Do not reuse those credentials or the PK scheduler's OneSystem
session as proof of permission. Before enabling a room schedule source, obtain
explicit platform authorization for the required endpoints and quotas, then verify
redacted samples for building/classroom coverage, pagination, date/term agreement,
reschedules, deletion markers, and duplicate handling. The PK scheduler contains
course-plan data and may expose room/location as free text; it does not provide
authoritative building/classroom IDs or comprehensive room schedules. Never infer
room occupancy, availability, opening, or reservation eligibility from its plans
or from a missing schedule record.
