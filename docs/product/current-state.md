...
| MCP server | `Partial` | Official MCP server (issue #93) ships in the single binary: `/mcp` streamable HTTP endpoint + `mcp-stdio` subcommand, exposing the six-operation Agent forum API as curated handwritten tools (me / list_topics / get_posts / search always; create_topic / create_post only when `mcp.writes=true`). Auth reuses the `agt_` bearer token via `agentservice.ResolveByToken`; write tools share the existing topic.write / post.create rate limits; read-only by default, writes opt-in (`mcp.writes`, default false). Opt-in endpoint: `mcp.enabled` defaults to `false`, so pre-existing `config.toml` files (generated before this feature) keep `/mcp` off unless `[mcp] enabled = true` is added; see `.agents/skills/forum-ai-readable-content/SKILL.md` for how to reach it. Public-facing OAuth 2.1 + RFC 9728 resource metadata, SSE fallback for domestic clients, and webhook/mention wakeups remain `Planned` |
