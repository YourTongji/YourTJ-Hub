...
- **Wiki 分站（论坛内嵌）**: wiki 已原生化进论坛单二进制（不再是独立 VitePress 静态站）——SSR
  `/wiki` 与 `/wiki/*path`（wiki.home/wiki.detail）、编辑审核流（PageManager/Admin）、管理端
  `/admin/wiki`；topics.topic_type 隔离论坛 feed 与搜索；审核通过向订阅者发 `wiki_updated` 通知。
...
| Wiki 分站 | `Current` | 论坛内嵌视图（同二进制）：SSR `/wiki` + `/wiki/*path`（wiki.home/wiki.detail）、公开 API（tree/namespaces/home/revisions）、登录写页面与修订审核流（PageManager/Admin）、管理端 `/admin/wiki`（PageManager 权限：命名空间/编辑者/树/修订队列）；topics.topic_type 隔离 feed/搜索（默认论坛搜索/feed/RSS/sitemap 排除 wiki）；审核通过发 `wiki_updated` 通知；OpenAPI wiki 域 16 操作已覆盖（TS 类型 + 手写 Dart mirror） |
