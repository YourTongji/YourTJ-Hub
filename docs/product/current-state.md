...
| Course reviews (课评) | `Partial` | Slice A landed (2026-08-10): cross-dialect course catalog schema (course/alias/term/offering/instructor/import-run/source-ref), offline `course-import` CLI with manifest checksum + dry-run + quarantine, PG read service, SSR `/courses` + `/courses/:courseId` and JSON `GET /api/forum/courses(/{courseId})` (OpenAPI-covered, route contract tests green); native review write, Meilisearch sync, and moderation remain `Planned` (Slices B–E) |
