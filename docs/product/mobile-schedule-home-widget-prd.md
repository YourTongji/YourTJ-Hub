...
Android 提供“下一节课”“今日课表”“课程时间线”三个 Widget kind；iOS 保持现有前两种。
...
### 8.3 “课程时间线” Android Widget

- 默认尺寸与“今日课表”一致，为 4x3（250dp × 180dp），支持横纵调整。
- 标题区显示日期、教学周和星期；右上角箭头在今天与明天之间切换，再次点击可切回，状态按小组件实例独立保存。
- 课程列表纵向手势滚动，滚动条保持隐式；不提供设置按钮。
- 每项按起始节次、开始时间和加粗课程名，再按结束节次、结束时间、教室与教师姓名排列；隐藏教师工号，不显示校区。
- 复用现有玻璃表面、动态浅深色、课程颜色槽、圆角和 YourTJ 品牌标识。
