...
class ThemeModeNotifier extends Notifier<ThemeMode> {
  ...
  Future<void> _restore() async {
    ...
    try {
      ...
      state = mode;
      // Restoration and later user choices share one native-write queue, so an
      // older platform response cannot win over the most recent selection.
      if (mode != ThemeMode.system) {
        _writes = _writes.then((_) => _syncNativeMode(mode));
      }
    } catch (_) {
      // 无本地存储(如测试环境)时静默保持默认。
    }
