...
- **运行配置**(`lib/src/app_config.dart`):经 `--dart-define` 注入 `YOURTJ_OIDC_ISSUER` / `YOURTJ_OIDC_CLIENT_ID` / `YOURTJ_API_BASE_URL`;默认 Casdoor `localhost:8001`,API baseUrl 为空时 Android 模拟器走 `10.0.2.2`。
