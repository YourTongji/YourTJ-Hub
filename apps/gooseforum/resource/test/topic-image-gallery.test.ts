...
describe('TopicImageGallery 组件', () => {
  ...
  test('多张图片时展示 1/N 计数徽章与左右切换控件', async () => {
    ...
    // 检查右翻页按钮
    const nextBtn = wrapper.find('button[aria-label="Next image"]')
    ...
    // 点击左翻页
    const prevBtn = wrapper.find('button[aria-label="Previous image"]')
  ...
  test('点击放大按钮可以唤起全屏大图预览 Lightbox', async () => {
    ...
    const zoomBtn = wrapper.find('button[aria-label="Zoom in"]')
    expect(zoomBtn.exists()).toBe(true)

    await zoomBtn.trigger('click')
    await flushPromises()

    const closeBtn = document.body.querySelector('button[aria-label="Close lightbox"]')
