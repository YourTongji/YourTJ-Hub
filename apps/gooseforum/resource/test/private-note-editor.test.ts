...
it('skips repositioning while closed and follows scroll while open', async () => {
  const wrapper = mount(PrivateNoteEditor, { props: { userId: 2, username: 'alice' }, global: { plugins: [plugin()] } })
  const rect = vi.spyOn(wrapper.get('[data-testid="private-note-edit"]').element, 'getBoundingClientRect')
  ...
  expect(rect).toHaveBeenCalled(); rect.mockClear()
  window.dispatchEvent(new Event('scroll')); await flushPromises()
  expect(rect).toHaveBeenCalled(); wrapper.unmount()
})
it('closes on outside pointer down and toggles from the trigger', async () => {
  const wrapper = mount(PrivateNoteEditor, { props: { userId: 2, username: 'alice' }, global: { plugins: [plugin()] } })
  const triggerButton = wrapper.get('[data-testid="private-note-edit"]')
  await triggerButton.trigger('click'); await flushPromises()
  ...
  expect(document.body.querySelector('[role="dialog"]')).not.toBeNull()
  await triggerButton.trigger('click'); await flushPromises()
  expect(document.body.querySelector('[role="dialog"]')).toBeNull(); wrapper.unmount()
})
it('closes on Escape and restores focus to the trigger', async () => {
  const wrapper = mount(PrivateNoteEditor, { props: { userId: 2, username: 'alice' }, global: { plugins: [plugin()] }, attachTo: document.body })
  const triggerButton = wrapper.get('[data-testid="private-note-edit"]')
  await triggerButton.trigger('click'); await flushPromises()
