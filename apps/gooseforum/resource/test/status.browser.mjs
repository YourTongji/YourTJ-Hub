...
const fixture = JSON.parse(readFileSync(new URL('../../../../packages/api-contract/fixtures/status-connected.json', import.meta.url), 'utf8'))
...
for (const [width, lang, theme] of [[320, 'zh', 'gf-light'], [390, 'de', 'gf-dark'], [1280, 'en', 'gf-dark'], [390, 'ja', 'gf-light']]) {
  test(`status stays usable at ${width}px in ${lang} / ${theme}`, async () => {
    ...
    try {
      await page.route('**/api/forum/status?*', async route => {
      ...
      await page.goto(`${origin}/assets/test/fixtures/browser/status.html?lang=${lang}&theme=${theme}`)
