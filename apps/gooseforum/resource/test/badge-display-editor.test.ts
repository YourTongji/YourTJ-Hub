...
test('persists the selected order independently of wearing a badge',async () => {
 ...
 await selectedCards()[1].find('button').trigger('keydown', { key: 'ArrowUp', altKey: true })
 expect(selectedCards().map(card => card.find('label span:last-child').text())).toEqual(['Badge 1','Badge 0'])
 await wrapper.find('#profile-badge-b2').setValue(true)
...
test('allows explicit empty display and enforces five selections',async () => {
 ...
 expect(wrapper.find('#profile-badge-b5').attributes('disabled')).toBeDefined()
 for (const badge of badges.slice(0,5)) await wrapper.find(`#profile-badge-${badge.code}`).setValue(false)
 expect(wrapper.find('#profile-badge-b5').attributes('disabled')).toBeUndefined()
 await wrapper.find('section > button').trigger('click');await flushPromises()
...
test('failed save retains the selection and allows retry',async () => {
 ...
 expect(wrapper.find('[aria-labelledby="badge-display-selected-label"]').findAll('[role="listitem"]')).toHaveLength(2)
 await wrapper.find('section > button').trigger('click');await flushPromises()
