...
} from '../src/admin/pages/wiki-sync-poll'
...
describe('isSyncTerminal (wiki sync polling, issue #290)', () => {
  ...
  test('长时间未进入终态在最大尝试次数前继续，达到上限则超时结束', () => {
    expect(hasSyncPollTimedOut(WIKI_SYNC_POLL_MAX_ATTEMPTS - 1)).toBe(false)
    expect(hasSyncPollTimedOut(WIKI_SYNC_POLL_MAX_ATTEMPTS)).toBe(true)
  })
})
