...
export function safeUrl(raw: string | null | undefined, policy: SafeUrlPolicy = 'site-link'): string {
  ...
  if (decoded.length > maxUrlLength || hasControl(decoded)) return ''
  if (decoded.startsWith('//')) return ''

  const match = schemeRe.exec(decoded)
  ...
  if (scheme === 'mailto' && policy === 'contact' && decoded.length > scheme.length + 1) {
    return value
  }
  return ''
}
