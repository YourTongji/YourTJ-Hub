...
import StatusUptime from '@/site/components/status/StatusUptime.vue'
  ...
  <div class="status-page">
    ...
    <section class="status-panel status-infra" aria-labelledby="infra-title">
      ...
      <div class="status-chart-panel resource-chart-panel" :aria-busy="loading">
        ...
        <StatusResourceChart v-if="historyServer?.historyAvailable && historyServer.history.length" :points="historyServer.history" :as-of="snapshot?.server.fetchedAt" :range="serverRange" />
...
@media (max-width: 639.98px) {
  .status-page { --status-chart-height: 136px; padding: 0 16px 28px; }
