...
// 将其置为 Running，生成新的 fencing token（lease_token，每次领取唯一、不可
// 复用）并以 processed_at 记录租约起点。返回领取后的行，其 LeaseToken 与
// ProcessedAt 为 DB 规范化后的值，供 RenewLease 的 CAS 续租使用。
...
// RenewLease 续租：仅当任务仍由同一 worker 持有（status=Running 且 lease_token
// 仍为 expectToken，即上次成功写入的 fencing token）时推进租约时间，并返回
// 续租后的租约值（DB 规范化读回，供下一次续租与终态写入的 CAS 使用）。任务
// 若已被回收并被其他 worker 重新领取，lease_token 已更换，续租失败
// （ok=false）—— 调用方应取消处理并放弃后续状态写入，避免与新持有者重复
// 执行外部副作用（租约 fencing）。
func RenewLease(id uint64, expectToken string) (ok bool, lease time.Time, token string, err error) {
	err = db.Connect().Transaction(func(tx *gorm.DB) error {
		res := tx.Table(tableName).
			Where("id = ? AND status = ? AND lease_token = ?", id, StatusRunning, expectToken).
		...
		if err := tx.Table(tableName).Select("processed_at, lease_token").Where("id = ?", id).Scan(&row).Error; err != nil {
			return err
		}
...
// UpdateStatusOwned 在仍持有租约（status=Running 且 lease_token=token）时更新
// 任务状态，用于成功/失败/重试终态写入；租约已被回收时静默跳过，避免覆盖新持有者。
func UpdateStatusOwned(id uint64, status uint8, token string, err error) error {
	updates := map[string]any{
		"status":       status,
		"processed_at": time.Now(),
	}
...
func IncrementRetryCountOwned(id uint64, token string) error {
	return builder().Exec(
		"UPDATE task_queue SET retry_count = retry_count + 1 WHERE id = ? AND status = ? AND lease_token = ?",
