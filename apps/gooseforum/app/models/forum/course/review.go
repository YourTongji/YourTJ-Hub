...
// 隔离窗口清理（courseservice.CleanupExpiredReviewsBatch）把 author_user_id 置 NULL
// 并清空 content：NULL 在唯一索引中彼此不冲突（SQL 标准，SQLite/PostgreSQL/MySQL
// 一致），因此同 offering 的多条已清理行可共存，且同用户可重新写评新建行。
// deleted_at 是隔离窗口锚点（普通时间列，非 GORM 软删语义）：作者删除时写入，
// 清理任务按 deleted_at 超过 ReviewCleanupWindow 扫描；全库统一用 status 表达
// 逻辑删除，避免 GORM 软删过滤隐式隐藏 deleted 行（Table 查询同样生效）。
