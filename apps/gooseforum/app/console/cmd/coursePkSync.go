...
凭证按优先级取：--onesystem-cookie 参数 > 受众专用环境变量（本科 ONESYSTEM_UNDERGRADUATE_COOKIE、研究生 ONESYSTEM_GRADUATE_COOKIE） > 旧 ONESYSTEM_COOKIE（仅本科） > 管理端对应设置（加密落库）。`,
...
func runCoursePkSync(cmd *cobra.Command, args []string) error {
	...
	cookie, err := pkservice.ResolveCookieForAudience(cookieFlag, audience)
	if err != nil {
		return err
	}
