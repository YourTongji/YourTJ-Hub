...
由 seed-stickers 命令导入数据库）。收录日期：**2026-09-15**。
导入与升级共用[预设清单](../../../bundles/stickerpresets/manifest.json)，图片和许可证保持在本目录。
...
- 导入保留清单中的来源分组；升级会为仍使用默认分组的系统预设恢复来源，
  不改动自定义分组、管理员上传、个人素材、表情名称或图片。
