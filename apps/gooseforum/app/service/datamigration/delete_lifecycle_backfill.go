...
// deleteLifecycleRecoveryWindow 与 contentdeleteservice.RecoveryWindow 对齐：
// 回填附件引用时给定 30 天恢复窗口，由 retention 定时任务清扫。
const deleteLifecycleRecoveryWindow = 30 * 24 * time.Hour
...
// 除状态机回填外，还同步把历史已删内容的附件引用置为 RECOVERING（30 天窗口），
// 让公开下载门禁对存量数据立即生效（PRD R5），而不是等到每日 retention 定时任务。
func BackfillDeleteLifecycleWithDB(conn *gorm.DB) DeleteLifecycleBackfillResult {
	result := DeleteLifecycleBackfillResult{}
	now := time.Now()
	expiresAt := now.Add(deleteLifecycleRecoveryWindow)

	var legacyTopicIDs []uint64
	if err := conn.Model(&topics.Entity{}).
		...
		Pluck("id", &legacyTopicIDs).Error; err != nil {
		result.Failed++
		result.LastFailed = "topics_scan:" + err.Error()
		slog.Error("scan legacy deleted topics failed", "err", err)
	} else if len(legacyTopicIDs) > 0 {
		...
		if topicRows.Error != nil {
			...
			hardenLegacyTargetFiles(conn, fileUsage.TargetTopic, legacyTopicIDs, expiresAt, &result)
		}
	}

	var legacyPostIDs []uint64
	if err := conn.Model(&posts.Entity{}).
		Unscoped().
		Where("deleted_at IS NOT NULL").
		Where("visibility_status = ?", posts.VisibilityActive).
		Where("retention_status = ?", posts.RetentionNormal).
		Pluck("id", &legacyPostIDs).Error; err != nil {
		result.Failed++
		result.LastFailed = "posts_scan:" + err.Error()
		slog.Error("scan legacy deleted posts failed", "err", err)
	} else if len(legacyPostIDs) > 0 {
		postRows := conn.Model(&posts.Entity{}).
			Unscoped().
			Where("id IN ?", legacyPostIDs).
			UpdateColumns(map[string]any{
		...
		if postRows.Error != nil {
			...
			hardenLegacyTargetFiles(conn, fileUsage.TargetPost, legacyPostIDs, expiresAt, &result)
		}
	}
...
// hardenLegacyTargetFiles 把一批已删内容的附件引用从 ACTIVE 置为 RECOVERING，
// 使公开下载门禁对历史存量数据立即生效。expiresAt 用于 retention 定时任务清扫。
// file_usages 表不存在时静默跳过（历史库可能尚未建表），不阻断状态机回填。
func hardenLegacyTargetFiles(conn *gorm.DB, targetType string, targetIDs []uint64, expiresAt time.Time, result *DeleteLifecycleBackfillResult) {
	if len(targetIDs) == 0 {
		return
	}
	if !conn.Migrator().HasTable(&fileUsage.Entity{}) {
		return
	}
	if err := conn.Model(&fileUsage.Entity{}).
		Where("target_type = ?", targetType).
		Where("target_id IN ?", targetIDs).
		Where("status = ?", fileUsage.UsageStatusActive).
		Updates(map[string]any{
			"status":     fileUsage.UsageStatusRecovering,
			"expires_at": expiresAt,
		}).Error; err != nil {
		result.Failed++
		result.LastFailed = "file_usages_backfill:" + err.Error()
		slog.Error("backfill file usages recovering failed", "targetType", targetType, "err", err)
	}
}
