...
func (s *Service) ExportCalendar(ctx context.Context, id uint64, times []pageConfig.ScheduleSectionTime) (CalendarExport, error) {
	...
	value, err := s.readPrivate(ctx, id, func(c Credentials) (any, error) {
		...
		return buildCalendar(normalize("calendar", calendar), normalize("timetable", timetable), times, namespace, time.Now())
	})
	if err != nil {
...
func buildCalendar(calendar, timetable Dataset, overrides []pageConfig.ScheduleSectionTime, namespace string, now time.Time) (CalendarExport, error) {
	...
	for _, course := range timetable.Events {
		...
		for _, week := range course.Weeks {
			...
			identity, marshalErr := json.Marshal([]string{namespace, begin.Format(time.DateOnly), course.Name, course.Teacher, course.Room, course.Campus, start.Format(time.RFC3339), finish.Format(time.RFC3339)})
			if marshalErr != nil {
