...
func apiRoute(ginApp *gin.Engine) {
	...
	// 用户（未验证邮箱、不打算/无法验证）仍可注销自有账号自救，控制器自带
	// ownership/密码/限流校验（issue #415 review P2）；普通写仍被
	// CheckWritableAccount 拦截。
	...
	adminApi.Group("", middleware.CheckPermission(permission.SiteManager)).
		...
		POST("sticker-save", UpButterReq(api.SaveSticker)).
		POST("sticker-delete", UpButterReq(api.DeleteSticker)).
		POST("sticker-import", api.ImportStickerPack).
