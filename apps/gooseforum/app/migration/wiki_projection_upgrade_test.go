...
// → MigrateWikiProjectionV21（回填投影/溯源 + editors 种子迁移 + DROP 两旧表）。
func exerciseWikiProjectionUpgrade(t *testing.T, db *gorm.DB) {
	...
	revisions := []wikiPageRevisions.Entity{
		...
		{PageId: page1.Id, RevisionNo: 2, Title: "简介v2", Content: "# 新内容", RenderedHTML: "<h1>新内容</h1>", Toc: `[{"level":1,"id":"new-content","text":"新内容"}]`, Status: wikiPageRevisions.StatusApproved, EditorId: 100},
...
// assertWikiProjectionSchema 断言新列/新表存在、两旧表已物理删除。
func assertWikiProjectionSchema(t *testing.T, db *gorm.DB) {
	...
	if db.Migrator().HasTable("wiki_page_revisions") {
		t.Error("wiki_page_revisions should be dropped after v21")
	}
	if db.Migrator().HasTable("wiki_namespace_editors") {
		t.Error("wiki_namespace_editors should be dropped after v21")
	}
}
...
func assertWikiProjectionBackfill(t *testing.T, db *gorm.DB, page1ID, page2ID uint64) {
	...
	if want := sha256HexForTest("# 新内容"); page1.ContentHash != want {
...
// → DROP 两旧表。核心断言：存量数据不丢、回填取最新 approved 修订、旧表物理删除。
...
// TestWikiProjectionV21CleansOrphanedEditorsOnPartialDrop 模拟 partial-drop 状态
//（review MEDIUM）：v21 曾删除 wiki_page_revisions 但在删除 wiki_namespace_editors 前
// 中断（版本未推进），重启后 revision 表缺失、editors 残留。迁移必须独立清理遗留表，
// 而不是用 revision 表存在性整体 no-op——否则 editors 表永久孤儿。
func TestWikiProjectionV21CleansOrphanedEditorsOnPartialDrop(t *testing.T) {
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{Logger: logger.Default.LogMode(logger.Silent)})
	if err != nil {
		t.Fatalf("open sqlite: %v", err)
	}
	// 构造 partial-drop 状态：editors 表仍在，revision 表已删。
	if err := db.AutoMigrate(&wikiNamespaceEditors.Entity{}); err != nil {
		t.Fatalf("migrate editors table: %v", err)
	}
	if err := db.Create(&wikiNamespaceEditors.Entity{Namespace: "school", UserId: 100, AddedBy: 1}).Error; err != nil {
		t.Fatalf("seed editor: %v", err)
	}
	if db.Migrator().HasTable("wiki_page_revisions") {
		t.Fatal("precondition failed: revision table should be absent")
	}
	...
	if db.Migrator().HasTable("wiki_namespace_editors") {
		t.Error("orphaned wiki_namespace_editors should be dropped on re-run after partial drop")
	}
}

// TestWikiProjectionUpgradeOnPostgreSQL 同上，PostgreSQL 版（PG 走 AutoMigrate ALTER
// COLUMN 加列，不重建表不丢数据）。依赖 YOURTJ_TEST_PG_URL，未设置时跳过。
func TestWikiProjectionUpgradeOnPostgreSQL(t *testing.T) {
